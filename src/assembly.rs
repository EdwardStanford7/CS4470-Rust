...
impl<'a> AsmFunction<'a> {
    ...
    fn pop_assert(&mut self, added_size: usize) -> bool {
        ...
        match size_opt {
            Some(size) => {
                size == self.shadow_stack.len() - added_size
            },
            None => false,
        }
    }
...
impl<'a> AssemblyGenerator<'a> {
    ...
    pub fn generate_assembly(
        ...
        for command in commands {
            ...
            main_function.print_shadow_stack();
            assert!(main_function.pop_assert_opt(vec![0,1]), "assertion failed for command {}\n", command.to_string());
        }
    ...
    fn generate_command(
        ...
        match command.node.as_ref() {
            CommandType::Show { expression } => {
                ...
                let expr_result = self.generate_expression(asm_function, expression, environment, false);
    ...
    fn fun_name(&mut self, environment: &TypeEnvironment<'a>, name: &&str, parameters: &Vec<(LValue<'a>, Type<'a>)>, return_type: &Type<'a>, statements: &Vec<Statement<'a>>) {
        let mut new_asm_function = AsmFunction::new();
        new_asm_function.push_label(&format!("{}", name));
        ...
        for param in parameters {
            match &param.1 {
                Type::Int | Type::Bool  => {
                    ...
                    self.offsets.insert(param.0.name.to_string(), ((AsmFunction::get_type_stack_size(&param.1) as usize, param.1.clone()), new_asm_function.stack_size as isize, false));
                    int_param_num += 1;
                }
                Type::Float  => {
                    ...
                    new_asm_function.add_shadow_type(&param.1);
                    self.offsets.insert(param.0.name.to_string(), ((size as usize, param.1.clone()), new_asm_function.stack_size as isize, false));
                }
                Type::Array { element_type, rank:_ } => {
                    ...
                    match &param.0.node {
                        LValueType::Array { indices } => {
                            ...
                            let size = AsmFunction::get_type_stack_size(&param.1);
                            self.offsets.insert(param.0.name.to_string(), ((size as usize, param.1.clone()), start, false));
                        ...
                        LValueType::Variable { .. } => {
                            let size  = AsmFunction::get_type_stack_size(&param.1);
                            self.offsets.insert(param.0.name.to_string(), ((size as usize, param.1.clone()), -(arr_param_size + 8), false));
                            arr_param_size += size as isize;
                        }
                    }
                }
    ...
    fn handle_let(
        ...
        match &variable.node {
            LValueType::Variable => {
                ...
                self.offsets.insert(variable.name.to_string(), (res,asm_function.stack_size as isize, !in_statement));
                asm_function.print_shadow_stack();
            ...
            LValueType::Array { indices } => {
                asm_function.push_assert();
                let res = self.generate_expression(asm_function, rvalue, environment, in_statement);
                self.offsets.insert(variable.name.to_string(), (res.clone(), asm_function.stack_size as isize, !in_statement));
                for (i,b_name) in indices.iter().enumerate() {
                    let loc = 8 * i as isize;
                    let place = 16 - res.0 as isize;
                    let stack_location = loc + place + asm_function.stack_size as isize;
    ...
    fn generate_expression(
        ...
        match expression.node.as_ref() {
            ...
            ExpressionType::Unop {
                ...
                asm_function.push_assert();
                asm_function.push_assert();
                let (size, typ) = self.generate_expression(asm_function, expression, environment, in_statement);
                asm_function.print_shadow_stack();
                assert!(asm_function.pop_assert(1), "\n\n{}", asm_function.body);
            ...
            ExpressionType::Binop {
                ...
                assert!(asm_function.pop_assert(1), "\n\n{}", asm_function.body);
                asm_function.push_assert();
                let (_, left_type) = self.generate_expression(asm_function, left, environment, in_statement);
                asm_function.print_shadow_stack();
                assert!(asm_function.pop_assert(1), "\n\n{}", asm_function.body);
            ...
            ExpressionType::ArrayLiteral { elements } => {
                ...
                for element in elements.iter().rev() {
                    let (size, _) = self.generate_expression(asm_function, element, environment, in_statement);
                    element_size = size;
                }
                asm_function.push_instruction(&format!("mov rdi, {}", elements.len() * element_size));
                asm_function.pad_shadow();
            ...
            ExpressionType::Call { function, arguments } => {
                ...
                match &expression.resolved_type {
                    ...
                    Type::Array { .. } => {
                        ...
                        asm_function.push_instruction(&format!("sub rsp, {}", AsmFunction::get_type_stack_size(&ret_type)));
                    }
                ...
                for arg in arguments.iter() {
                    match &arg.resolved_type {
                        Type::Int | Type::Bool  => {
                            asm_function.push_instruction(&format!("pop {}", INT_REGS[int_arg_num]));
                            int_arg_num += 1;
                            asm_function.remove_shadow();
                        }
                        Type::Float => {
                            asm_function.push_instruction(&format!("movsd {}, [rsp]", FLO_REGS[flo_arg_num]));
                ...
                match &expression.resolved_type {
                    Type::Int | Type::Bool | Type::Float => {}
                    Type::Array { .. } => {
                        asm_function.push_instruction(&format!("lea rdi, [rsp + {}]", arr_arg_size));
                    }
                    _ => unimplemented!("\n\nfailure for call return type {}", ret_type.to_string())
                }
                asm_function.push_instruction(&format!("call _{}", function.to_string()));
                for arg in arguments.iter() {
                    match &arg.resolved_type {
                        Type::Array { element_type, rank:_ } => {
                            match element_type.as_ref() {
    ...
    fn handle_variable(
        ...
        match expression.resolved_type {
            Type::Int | Type::Bool | Type::Float => {
                ...
                asm_function.push_instruction(&format!("mov r10, [{} - {}]",var_offset_reg, offset - 8));
            ...
            Type::Array { element_type: _, rank } => {
                ...
                for i in (0..rank+1).rev() {
                    ...
                    asm_function.push_instruction(&format!("mov r10, [{} - {} + {}]", var_offset_reg, bounds_offset, loc));
    ...
    fn generate_int_op(
        ...
        match operator {
            "+" => {
                asm_function.push_instructions(vec!["pop rax", "pop r10", "add rax, r10", "push rax"]);
                asm_function.remove_shadow();
                asm_function.remove_shadow();
                asm_function.add_shadow_type(&Type::Int);
            },
            "-" => {
                asm_function.push_instructions(vec!["pop rax", "pop r10", "sub rax, r10", "push rax"]);
                asm_function.remove_shadow();
                asm_function.remove_shadow();
                asm_function.add_shadow_type(&Type::Int);
            },
            "*" => {
                asm_function.push_instructions(vec!["pop rax", "pop r10", "imul rax, r10", "push rax"]);
                asm_function.remove_shadow();
                asm_function.remove_shadow();
                asm_function.add_shadow_type(&Type::Int);
            },
            "/" | "%" => {
                ...
                asm_function.pad_shadow();
                asm_function.push_instruction(&format!("lea rdi, [rel {}]", self.get_constant(AssemblyValue::String(
                    if operator == "/" {
                        "divide by zero"
                    } else {
                        "mod by zero"
